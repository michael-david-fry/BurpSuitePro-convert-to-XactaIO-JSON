#!/usr/bin/env python3
"""
Burp Suite XML -> Xacta JSON Converter (v3 - Template-Ordered Output)
...
Changes from v2:
- Matches output structure to the Documentation/XIO JSON Template example
- Emits top-level JSON array of assets (instead of {"assets": [...]})
- Enforces deterministic key ordering for asset and testResults fields
- Preserves v2 multi-host grouping, audit annotations, and sanitization
...
# Minimal Xacta JSON schema (template-aligned: top-level array of assets)
XACTA_JSON_SCHEMA = {
    "type": "array",
    "items": {
        "type": "object",
        "required": ["dataSource", "hostName", "scanDate"],
        "properties": {
...
# Ordered to mirror `Xacta io Documentation/XIO JSON Template.json`
ASSET_FIELD_ORDER = [
    "dataSource",
    "hostName",
    "assetRole",
...
def ordered_subset(source: Dict[str, Any], order: List[str]) -> Dict[str, Any]:
    return {key: source[key] for key in order if key in source}


def render_xacta_payload(assets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for raw_asset in assets:
        ...
        sanitized_asset = ordered_subset(raw_asset, ASSET_FIELD_ORDER)
        ...
        if test_results is not None:
            ...
            for result in test_results:
                if not isinstance(result, dict):
                    continue
                cleaned_result = ordered_subset(result, TEST_RESULT_FIELD_ORDER)
        ...
        rendered_assets.append(ordered_subset(sanitized_asset, ASSET_FIELD_ORDER))
...
def validate_xacta_json(data: List[Dict[str, Any]]) -> None:
